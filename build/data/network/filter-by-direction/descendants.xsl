...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="network" priority="10">			
		<xsl:next-match>
			<xsl:with-param name="filtered-edges" as="element()*" tunnel="yes">
				<xsl:apply-templates select="edges/edge/node[@role = 'source'][@ref = $anchor-person-id]" mode="filter" />
			</xsl:with-param>
		</xsl:next-match>		
	</xsl:template>
	...
	<xsl:template match="edge/node" mode="filter">
		<xsl:variable name="child-id" select="parent::edge/node[@role = 'target']/@ref" as="xs:string" />
			
		<xsl:apply-templates select="parent::edge" />
		<xsl:apply-templates select="ancestor::edges[1]/edge/node[@role = 'source'][@ref = $child-id]" mode="filter" />
	</xsl:template>
