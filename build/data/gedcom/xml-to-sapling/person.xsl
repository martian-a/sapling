...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="individual" mode="people">
        ...
        <person id="{@id}">
            <xsl:apply-templates select="birth[1]/date" mode="person-year" />
    ...
    <xsl:template match="individual/birth/date" mode="person-year">
        <xsl:attribute name="year"><xsl:value-of select="tokenize(., ' ')[position() = last()]" /></xsl:attribute>
    </xsl:template>
