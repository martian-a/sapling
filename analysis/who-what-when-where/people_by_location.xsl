...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
    	<xsl:variable name="html-document-title" as="xs:string">People by Location</xsl:variable>
        <html>
	...
	<xsl:template match="data/locations">
		<ul class="locations">
			<xsl:apply-templates select="location[not(within)]" />
		</ul>
	</xsl:template>
	...
	<xsl:template match="location">
		<li class="location">
			...
			<xsl:variable name="locations-within" select="key('child-locations', @id)" as="element()*" />
			<xsl:if test="$locations-within">
				<ul>
					<xsl:apply-templates select="$locations-within">
