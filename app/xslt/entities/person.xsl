	...
	<xsl:template match="related" mode="family.network-graph">
		...
		<div class="network-graph">
			...
			<div class="network-visualisation">
				<object data="{if ($static = 'true') then concat($normalised-path-to-images, 'network-graphs/svg/') else $normalised-path-to-view-svg}person/{$subject-id}{if ($static = 'true') then '.svg' else ()}" type="image/svg+xml">
					<img src="{if ($static = 'true') then concat($normalised-path-to-images, 'network-graphs/svg/') else $normalised-path-to-view-svg}person/{$subject-id}{if ($static = 'true') then '.svg' else ()}" />
				</object>
			</div>
		</div>
